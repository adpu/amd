...
?>
   ...
   <?php 
    ...
    foreach ( $lastposts as $post ) :
      setup_postdata( $post );
    $current_metatitle=get_post_meta( get_the_ID(), '_amd_metatitle', true );
    $current_metadesc=get_post_meta( get_the_ID(), '_amd_metadescription', true );
    $total_char_current_metatitle=strlen($current_metatitle);
    $total_char_current_metadesc=strlen($current_metadesc);
  ?>
   <tr>
	<td class="title column-title" data-colname="Title"><?php the_title(); ?></td><td class="metatitle column-metatitle" data-colname="Metatitle"><?php echo $current_metatitle;?> <span class="badge 
...
<a href="?page=scan-meta-tags&tab=posts&current_page=<?php echo $current_page-1;?>" class="amd_nav_before">Previous posts</a>
<?php }?>
